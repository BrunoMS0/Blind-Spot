// Visión: cono de VISION_RANGE casillas y VISION_ANGLE_DEG de apertura, con línea de vista.
import { VISION_ANGLE_DEG, VISION_RANGE } from "./config";
import { DIRS, solid, type Level } from "./level";
...
export function canSee(level: Level, s: Pick<GameState, "vault">, viewer: Viewer, target: Vec): boolean {
...
export function visibleTiles(level: Level, s: Pick<GameState, "vault">, viewer: Viewer): Vec[] {
  const out: Vec[] = [];
